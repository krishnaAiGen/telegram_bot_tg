...
async def get_grok_response(content: str, model: str = "grok-3-latest") -> str:
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant that provides concise, factual answers based on the latest available information."},
    ...
    async with aiohttp.ClientSession() as session:
        try:
            print(f"[GROK] Sending request to model '{model}'...")
            ...
            # Return a string that includes "Error" so our handler knows it failed.
